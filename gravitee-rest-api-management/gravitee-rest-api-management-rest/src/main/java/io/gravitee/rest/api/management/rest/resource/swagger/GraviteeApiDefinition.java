...
public class GraviteeApiDefinition implements ReaderListener {
    ...
    public void beforeScan(OpenApiReader reader, OpenAPI openAPI) {
        openAPI.info(new Info()
                ...
                new Server().url("http://localhost:8083/management")
        ));
    }
    ...
    public void afterScan(OpenApiReader reader, OpenAPI openAPI) {
        ...
        openAPI.getPaths().values().stream()
                ...
                .map(Operation::getTags).filter(Objects::nonNull).flatMap(Collection::stream)
        ...
        components.addSecuritySchemes(TOKEN_AUTH_SCHEME, new SecurityScheme()
