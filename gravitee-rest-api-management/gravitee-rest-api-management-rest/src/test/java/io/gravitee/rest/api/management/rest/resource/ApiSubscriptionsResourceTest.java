...
public class ApiSubscriptionsResourceTest extends AbstractResourceTest {
    ...
    public void shouldCreateSubscription() {
        ...
        assertEquals(
            envTarget().path(API).path("subscriptions").path(SUBSCRIPTION).getUri().toString(),
    ...
    public void shouldRenewApiKeyForApiSubscription() {
        ...
        assertEquals(CREATED_201, response.getStatus());
        assertEquals(
            envTarget().path(API).path("subscriptions").path(SUBSCRIPTION).path("keys").path(NEW_APIKEY).getUri().toString(),
            response.getHeaders().getFirst(HttpHeaders.LOCATION)
        );
    }
}
